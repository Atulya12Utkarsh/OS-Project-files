...
scheduler(void)
{
  ...
  for(;;){
    ...
    struct proc *batch[NPROC];
    int batch_count = 0;

    // STEP 1: collect RUNNABLE (NO LOCK HELD)
    for(struct proc *p = proc; p < &proc[NPROC]; p++){
      if(p->state == RUNNABLE){
        p->est_time = p->rtime + 1;
        batch[batch_count++] = p;
      }
    }
    ...
    // STEP 2: sort by est_time (descending)
    for(int i = 0; i < batch_count; i++){
      for(int j = i+1; j < batch_count; j++){
        if(batch[j]->est_time > batch[i]->est_time){
          struct proc *tmp = batch[i];
          batch[i] = batch[j];
          batch[j] = tmp;
        }
      }
    ...
    // STEP 3: tuple-based slack ordering
    int NCORES = NCPU;
    struct proc *final_list[NPROC];
    int final_count = 0;
    ...
    int start = i;
    int end = i + NCORES;
    if(end > batch_count) end = batch_count;

    for(int k = start; k < end; k++){
      final_list[final_count++] = batch[k];
    }

    i = end;
    }

    // STEP 4: run processes (LOCK HERE ONLY)
    int found = 0;

    for(int i = 0; i < final_count; i++){
      struct proc *p = final_list[i];
